...
# Matches backend.py — configurable so a future Groq model deprecation
# only needs a .env change, not a code change.
...
client = MultiServerMCPClient(
    {
        ...
        "weather": {
            ...
            # Uses the same Python interpreter running the main app.
...
async def _get_server_tool(
    ...
    if server_name == "tavily":
        ...
        if shutil.which("uvx") is None:
            raise RuntimeError(
                "uvx was not found. Install uv (pip install uv) and "
                "reopen the terminal, then run `uvx --version`."
            )
